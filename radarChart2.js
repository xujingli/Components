...
function RadarChart(id, data, options) {
  ...
  let allAxis = (data[0].map(function (i, j) {
    ...
    radius = Math.min(cfg.w / 2, cfg.h / 2), //最外层圆的半径
    Format = d3.format('%'), // 每一个圆圈代表的数字的格式， 可以换成其他，比如 ".2f"
  ...
  axisGrid.selectAll(".levels")
    .data(d3.range(1, (cfg.levels + 1)).reverse()) // // [5,4,3,2,1]
    .enter()
    .append("circle")
    .attr("class", "gridCircle")
    .attr("r", function (d, i) {
      return radius / cfg.levels * d; // 目前levels为 5
    })
    .style("fill", "#CDCDCD")
    .style("stroke", "#CDCDCD")
  ...
  blobWrapper
    ...
    .on('mouseover', function (d, i) {
      ...
      d3.selectAll(".radarArea")
        ...
        .style("fill-opacity", cfg.opacityArea); // 将所有的其他blobs变为原来的透明度
      //Bring back the hovered over blob
      d3.select(this)
        .transition().duration(200)
        .style("fill-opacity", 0.7); // 改变当前区域的透明度
    })
